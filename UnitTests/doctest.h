...
#if 1//DOCTEST_CLANG && !DOCTEST_ICC
...
// MSVS 2015 :(
    ...
    using insert_hack_t = insert_hack<T, has_global_insertion_operator<T>::value>;
        ...
        struct StringMakerBase {
            template <typename T>
            static String convert(const DOCTEST_REF_from(T)) {
        ...
        struct StringMakerBase<true> {
            template <typename T>
            static String convert(const DOCTEST_REF_from(T) in) {
                return toStream(in);
            }
        };
    } // namespace detail
...
#define DOCTEST_STRINGIFY(...) toString(__VA_ARGS__)
    ...
    String toString(const DOCTEST_REF_from(T) value) {
...
#undef DOCTEST_APPROX_PREFIX
    ...
    template<class T> struct can_use_op : public not_char_pointer<typename decay_array<T>::type> {};
        ...
        String stringifyBinaryExpr(const DOCTEST_REF_from(L) lhs, const char* op,
                                   const DOCTEST_REF_from(R) rhs) {
...
#define DOCTEST_RELATIONAL_OP(name, op)                                                            \
    template <typename L, typename R>                                                              \
    DOCTEST_COMPARISON_RETURN_TYPE name(const DOCTEST_REF_from(L) lhs,                             \
                                        const DOCTEST_REF_from(R) rhs) {                           \
        return lhs op rhs;                                                                         \
    }
...
// this is needed only for MSVC 2015
        ...
        // clang-format off
        template <int, class L, class R> struct RelationalComparator     { bool operator()(const DOCTEST_REF_from(L),     const DOCTEST_REF_from(R)    ) const { return false;        } };

#define DOCTEST_BINARY_RELATIONAL_OP(n, op) \
    template <class L, class R> struct RelationalComparator<n, L, R> { bool operator()(const DOCTEST_REF_from(L) lhs, const DOCTEST_REF_from(R) rhs) const { return op(lhs, rhs); } };
        // clang-format on
        ...
        struct DOCTEST_INTERFACE ResultBuilder : public AssertData
        {
            ...
            DOCTEST_NOINLINE bool binary_assert(const DOCTEST_REF_from(L) lhs,
                                                const DOCTEST_REF_from(R) rhs) {
            ...
            template <typename L>
            DOCTEST_NOINLINE bool unary_assert(const DOCTEST_REF_from(L) val) {
                m_failed = !val;
...
#define DOCTEST_ASSERT_IN_TESTS(decomp)                                                            \
    ...
    throwException()
        ...
        DOCTEST_NOINLINE bool binary_assert(assertType::Enum at, const char* file, int line,
                                            const char* expr, const DOCTEST_REF_from(L) lhs,
                                            const DOCTEST_REF_from(R) rhs) {
        ...
        DOCTEST_NOINLINE bool unary_assert(assertType::Enum at, const char* file, int line,
                                           const char* expr, const DOCTEST_REF_from(L) val) {
            bool failed = !val;
...
#define DOCTEST_from_IN_TRY(x) x;
#else // DOCTEST_CONFIG_NO_TRY_CATCH_IN_ASSERTS
#define DOCTEST_from_IN_TRY(x)                                                                     \
    try {                                                                                          \
        x;                                                                                         \
...
#define DOCTEST_ASSERT_IMPLEMENT_2(assert_type, ...)                                               \
    ...
    DOCTEST_from_IN_TRY(DOCTEST_RB.setResult(                                                      \
...
#define DOCTEST_UNARY_ASSERT(assert_type, ...)                                                     \
    DOCTEST_FUNC_SCOPE_BEGIN {                                                                     \
        doctest::detail::ResultBuilder DOCTEST_RB(doctest::assertType::assert_type, __FILE__,      \
                                                   __LINE__, #__VA_ARGS__);                        \
        DOCTEST_from_IN_TRY(DOCTEST_RB.unary_assert(__VA_ARGS__))                                  \
        DOCTEST_ASSERT_LOG_REACT_RETURN(DOCTEST_RB);                                               \
    } DOCTEST_FUNC_SCOPE_END
...
#define DOCTEST_RELATIONAL_OP(name, op)                                                            \
    template <typename L, typename R>                                                              \
    bool name(const DOCTEST_REF_from(L) lhs, const DOCTEST_REF_from(R) rhs) { return lhs op rhs; }
